...
import bitwolf.{SimpleMovingAverage, ExponentialMovingAverage}
...
class ExponentialMovingAverageSpec extends FlatSpec {
	"EMA" should "equal SMA for first n values" in {
		val n = (Random.nextFloat * 50).toInt + 10
		val sma10 = new SimpleMovingAverage(n)
		val ema10 = new ExponentialMovingAverage(n)
	...
	"EMA" should "match the EMA10 example given" in {
		...
		val ema10 = new ExponentialMovingAverage(10)

		inputPrices.take(9).foreach{ ema10.update(_) }

		inputPrices.drop(9).zip(expectedEMAs).foreach{
...
class SimpleMovingAverageSpec extends FlatSpec {
	"SMA" should "match the SMA10 example given" in {
		...
		inputPrices.take(9).foreach{ sma10.update(_) }

		inputPrices.drop(9).zip(expectedSMAs).foreach{
			case (price: Double, expected: Double) =>
				sma10.update(price)
